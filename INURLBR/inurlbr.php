...
--tor-random Enables the TOR function, each usage links an unique IP.
 ...
 --comand-vul Every vulnerable URL found will execute this command parameters.
     ...
     Usage:   --comand-vul 'nmap -Pn -p 1-8080 --script http-enum --open _TARGET_'
 ...
 --comand-all Use this commmand to specify a single command to EVERY URL found.
     ...
     Usage:   --comand-all 'nmap -Pn -p 1-8080 --script http-enum --open _TARGET_'
   ...
   _TARGETIP_ will be replaced by the target ip address found.
...
function __comando($comando, $alvo) {
    if (!is_NULL($comando)) {
        (strstr($comando, '_TARGET_') || strstr($comando, '_TARGETFULL_') || strstr($comando, '_TARGETIP_') || strstr($comando, '_EXPLOIT_') ? NULL : __sair(__bannerLogo() . "\033[1;37m0x\033[1;33mSET PARAMETER - comand correctly\033[0m\n"));

        $comandoCor = str_replace("_TARGET_", "\033[0;34m" . __filtroHostname($alvo) . "\033[1;34m", $comando);
        $comandoCor = str_replace('_TARGETIP_', "\033[0;37m{$_SESSION['config']['server_ip']}\033[1;34m", $comandoCor);
        $comandoCor = str_replace('_TARGETFULL_', "\033[0;36m{$alvo}\033[1;34m", $comandoCor);
        $comandoCor = str_replace("_EXPLOIT_", "\033[0;35m{$_SESSION['config']['exploit-comand']}\033[1;34m", $comandoCor);

        $comando = str_replace("_TARGET_", __filtroHostname($alvo), $comando);
        $comando = str_replace('_TARGETIP_', $_SESSION['config']['server_ip'], $comando);
        $comando = str_replace('_TARGETFULL_', $alvo, $comando);
        $comando = str_replace("_EXPLOIT_", $_SESSION['config']['exploit-comand'], $comando);

        echo "\n\033[1;34m[*]__\033[0m\n";
        echo "     \033[1;34m|EXTERNAL COMMAND:: {$comandoCor}\033[1;35m\n";
